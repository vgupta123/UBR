...
import ast
from load_db_movies import Summary_Text,Score,UserId
sampling=ast.literal_eval(open('../preprocess/sampling_movies.txt','r').read())
for name,entity in izip(['Summary_Text'],[Summary_Text]):
	...
	for name2,algo in izip(['UBR-1','UBR-2'],[1,2]):
		print name2
		if algo==1:
			from load_db_movies import transformed_score,user_mean,user_std
		else:
			from load_db_movies_2 import transformed_score,user_bias
		yt_train=[]
	        i=-1
        	for text in entity:
               		i+=1
